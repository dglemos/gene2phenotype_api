...
class G2PStableID(models.Model):
    """
    Represents a stable identifier for a G2P record.
    """
...
class CurationData(models.Model):
    ...
    stable_id = models.ForeignKey(
        "G2PStableID", on_delete=models.PROTECT, db_column="stable_id"
    )
...
class LocusGenotypeDisease(models.Model):
    ...
    genotype = models.ForeignKey(
        "Attrib", related_name="genotype", on_delete=models.PROTECT
    )
    disease = models.ForeignKey("Disease", on_delete=models.PROTECT)
    mechanism = models.ForeignKey(
        "CVMolecularMechanism",
        related_name="mechanism",
        on_delete=models.PROTECT,
        null=False,
    )
    mechanism_support = models.ForeignKey(
        "CVMolecularMechanism",
        related_name="mechanism_support",
        on_delete=models.PROTECT,
        null=False,
    )
    confidence = models.ForeignKey(
        "Attrib", related_name="confidence", on_delete=models.PROTECT
    )  # confidence value
    confidence_support = models.TextField(
        null=True, default=None
    )  # text summary to support the confidence value
...
class LGDMolecularMechanismSynopsis(models.Model):
    id = models.AutoField(primary_key=True)
    lgd = models.ForeignKey(
        "LocusGenotypeDisease", on_delete=models.PROTECT, null=False
    )
    synopsis = models.ForeignKey(
        "CVMolecularMechanism", related_name="synopsis", on_delete=models.PROTECT
    )
    synopsis_support = models.ForeignKey(
        "CVMolecularMechanism",
        related_name="synopsis_support",
        on_delete=models.PROTECT,
    )
    is_deleted = models.SmallIntegerField(null=False, default=False)
    history = HistoricalRecords()
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["lgd"])]
...
class LGDVariantType(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    lgd = models.ForeignKey("LocusGenotypeDisease", on_delete=models.PROTECT)
    variant_type_ot = models.ForeignKey(
        "OntologyTerm", related_name="variant_type", on_delete=models.PROTECT
    )
...
class LGDVariantGenccConsequence(models.Model):
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["variant_consequence"])]
...
class CVMolecularMechanism(models.Model):
    ...
    subtype = models.CharField(
        max_length=100, choices=choices_evidence_types, null=True
    )
...
class LGDPublication(models.Model):
    ...
    consanguinity = models.ForeignKey(
        "Attrib",
...
class Locus(models.Model):
    ...
    class Meta:
        db_table = "locus"
        indexes = [models.Index(fields=["name"]), models.Index(fields=["type"])]
...
class LocusAttrib(models.Model):
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["value"]), models.Index(fields=["attrib_type"])]
...
class DiseaseOntologyTerm(models.Model):
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["ontology_term"])]
...
class PhenotypePublication(models.Model):
    ...
    phenotype = models.ForeignKey("OntologyTerm", on_delete=models.PROTECT)
    publication = models.ForeignKey("Publication", on_delete=models.PROTECT, null=True)
    pheno_count = models.IntegerField(
        null=True
    )  # Probably not necessary - maybe a comment
    is_deleted = models.SmallIntegerField(null=False, default=False)
    history = HistoricalRecords()

    class Meta:
        db_table = "phenotype_publication"
        indexes = [models.Index(fields=["phenotype"])]
...
class Panel(models.Model):
    ...
    description = models.CharField(
        max_length=255, null=False, default="before_G2P_2025"
    )  # default was added because this is the default settings for django when changing from null=True to null=False
...
class GeneStats(models.Model):
    ...
    source = models.ForeignKey("Source", on_delete=models.PROTECT)
    description_attrib = models.ForeignKey(
        "Attrib", default="", on_delete=models.PROTECT
    )
...
class GeneDisease(models.Model):
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["gene"]), models.Index(fields=["disease"])]
...
class LGDMutationConsequenceFlag(models.Model):
    ...
    lgd = models.ForeignKey("LocusGenotypeDisease", on_delete=models.PROTECT)
    mutation_consequence_flag = models.ForeignKey(
        "Attrib", related_name="mutation_consequence_flag", on_delete=models.PROTECT
    )
